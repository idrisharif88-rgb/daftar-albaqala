...
describe('buildMessage', () => {
  it('a debt on a زبون, in riyals', () => {
    ...
    expect(msg).toBe(
      'بقالة الأمل\n' +
      'تسجيل دين 5,000 ريال\n' +
      '\n' +
      'رصيدك الآن: 20,000 ريال عليك\n' +
      'عشرون ألف ريال'
    );
  });
  ...
  it('goods taken on credit from a صاحب متجر read as a debt', () => {
    ...
    expect(msg).toBe(
      'إدريس\n' +
      'تسجيل دين 3,000 ريال\n' +
      '\n' +
      'رصيدك الآن: 8,000 ريال لك\n' +
      'ثمانية آلاف ريال'
    );
  });
  ...
  it('a شريك is addressed directly — «أخذت منك», not «أخذت منه»', () => {
    ...
    expect(msg.split('\n')[1]).toBe('أخذت منك 5,000 ريال');
    expect(msg).toContain('رصيدك الآن: 5,000 ريال لك');
  });
  ...
  it('spells a foreign-currency entry out in riyals, with the rate', () => {
    ...
    expect(msg).toBe(
      'بقالة الأمل\n' +
      'تسجيل دين 100 ر.س\n' +
      '≈ 14,000 ريال (سعر الصرف 140)\n' +
      '\n' +
      '20,000 ريال عليك\n' +
      '100 ر.س عليك\n' +
      '≈ 14,000 ريال\n' +
      'رصيدك الآن: 34,000 ريال عليك\n' +
  ...
  it('marks a total that had to skip an unpriced currency', () => {
    ...
    expect(msg).toContain('رصيدك الآن: 1,000 ريال عليك (عدا ما لم يُحدَّد سعره)');
